...
export default function () {
  ...
  const quickResponse = http.get(
    `${baseUrl}/api/search/quick?q=${encodeURIComponent(query)}&organizationId=${orgId}`,
    {
      headers,
  ...
  const fullResponse = http.get(
    `${baseUrl}/api/search?q=${encodeURIComponent(query)}&organizationId=${orgId}`,
    {
      headers,
