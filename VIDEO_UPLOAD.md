...
The video upload functionality allows users to upload video files to their workspace with automatic processing, thumbnail generation, and storage using Cloudflare R2.
...
- **Navigation**: Seamless integration with existing workspace navigation
...
2. Click the "Upload Video" button from:
   - Main workspace page (in "New this week" section)
   - "My Videos" page (top right corner)
   - Or directly visit `/{workspace}/upload`
...
- **"Missing required fields"**: Title, workspace, or author not provided
