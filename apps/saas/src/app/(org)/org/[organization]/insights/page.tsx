...
async function InsightsLoader({ params }: { params: Promise<{ organization: string }> }) {
  ...
  return <InsightsDashboard organizationId={organization.id} />;
}
...
export default function InsightsPage({ params }: { params: Promise<{ organization: string }> }) {
  return (
    <Suspense fallback={<InsightsPageSkeleton />}>
      <InsightsLoader params={params} />
    </Suspense>
  );
}
