...
interface CollectionCardProps {
  collection: CollectionWithVideoCount & { progress?: CollectionProgressWithDetails };
  organization: string;
  onEdit?: (collection: CollectionWithVideoCount) => void;
  onDelete?: (collectionId: string) => void;
  /** Mark as priority for above-the-fold images */
  priority?: boolean;
}

export function CollectionCard({ collection, organization, onEdit, onDelete, priority = false }: CollectionCardProps) {
  ...
  return (
    <Card className="group hover:border-primary transition-colors overflow-hidden">
      ...
      <CardContent className="p-4">
        <Link href={`/org/${organization}/collections/${collection.id}`}>
          <h3 className="font-semibold text-lg line-clamp-1 hover:text-primary transition-colors">{collection.name}</h3>
        </Link>
        ...
        {hasProgress && collection.progress?.lastVideo && (
          <Link
            href={`/org/${organization}/videos/${collection.progress.lastVideo.id}`}
