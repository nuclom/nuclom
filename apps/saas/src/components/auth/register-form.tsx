...
export function RegisterForm({ redirectTo }: RegisterFormProps) {
  ...
  const loginUrl = `/login${finalRedirectTo !== '/onboarding' ? `?redirectTo=${encodeURIComponent(finalRedirectTo)}` : ''}`;
    ...
    <Card className="w-full max-w-md mx-auto">
      <CardHeader className="space-y-1 pb-4">
        <div className="flex items-center justify-between">
          ...
          <Link href={loginUrl} className="text-sm text-muted-foreground hover:text-primary transition-colors">
            Sign in instead
          </Link>
        </div>
