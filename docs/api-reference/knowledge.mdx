...
description: 'Access decisions, relationships, and organizational knowledge'
...
Updates an existing decision (partial update allowed).
...
```bash Request
GET /api/knowledge/timeline?organizationId=uuid&startDate=2024-01-01&endDate=2024-01-31
```
...
```bash Request
GET /api/knowledge/context?organizationId=uuid&artifactType=github:pr&artifactId=123
```
...
```bash Request
GET /api/knowledge/graph?organizationId=uuid
```
