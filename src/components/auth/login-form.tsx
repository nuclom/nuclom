...
export function LoginForm({ redirectTo }: LoginFormProps) {
  ...
  const registerUrl = `/register${finalRedirectTo !== '/onboarding' ? `?redirectTo=${encodeURIComponent(finalRedirectTo)}` : ''}`;
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            ...
            {lastLoginMethod === 'google' && (
              <Badge className="absolute -top-2 -right-2 px-2 py-0.5 text-xs">Last</Badge>
            )}
          </div>
          <div className="relative">
            <Button
