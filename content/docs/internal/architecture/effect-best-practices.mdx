---
title: "Effect-TS Best Practices"
description: "Patterns and guidelines for Effect-TS based on official documentation and community standards"
icon: "book-open"
---
...
<Tabs>
  <Tab title="Context.Tag (Standard)">
  ...
  <Tab title="Effect.Service (Simplified)">
    ...
    // Usage: ApiService.Default provides the complete layer
    ...
    <Note>
    `Effect.Service` requires object types. For primitives like strings, use `Context.Tag`.
    </Note>
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  <Card title="Automatic _tag property" icon="tag">
    Includes `_tag` for pattern matching
  </Card>
  <Card title="Stack Traces" icon="list-tree">
    Collects stack traces for debugging
  </Card>
  <Card title="catchTag Integration" icon="code-merge">
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 3. Execute with full layer and handle exit
...
export async function POST(request: NextRequest) {
  ...
  const exit = await runApiEffect(effect);
  return handleEffectExitWithOptions(exit, { status: 201 }); // 201 Created
}
```
...
<AccordionGroup>
  <Accordion title="1. Forgetting Effects Are Lazy">
    ```typescript
  ...
  <Accordion title="2. Executing Effects Inside Effect.gen">
    ```typescript
  ...
  <Accordion title="3. Long andThen/flatMap Chains">
    ```typescript
  ...
  <Accordion title="4. Forgetting to Provide Required Layers">
    ```typescript
  ...
  <Accordion title="5. Resource Leaks">
    ```typescript
  ...
  <Accordion title="6. Unbounded Concurrency">
    ```typescript
  ...
  <Accordion title="7. Race Conditions on Shared State">
    ```typescript
    // BAD: Direct mutation
    let counter = 0;
...
<Note>
Even when using direct `db`, wrap in `Effect.tryPromise` with proper error types.
</Note>
...
<Warning>
New API routes should default to Effect patterns unless there's a specific reason not to.
</Warning>
...
<Steps>
  <Step title="User-facing APIs with direct db access">
    Highest priority - most user impact
  </Step>
  <Step title="APIs with complex error scenarios">
    Type safety benefits most here
  </Step>
  <Step title="Internal/admin APIs">
    Lower traffic, can iterate
  </Step>
  <Step title="Health checks and webhooks">
    Lowest priority - already working
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Effect Official Documentation" href="https://effect.website/docs" icon="book">
    Official Effect-TS documentation
  </Card>
  <Card title="TypeOnce Effect Course" href="https://www.typeonce.dev/course/effect-beginners-complete-getting-started" icon="graduation-cap">
    Beginners course
  </Card>
  <Card title="EffectPatterns Repository" href="https://github.com/PaulJPhilp/EffectPatterns" icon="github">
    Community patterns
  </Card>
  <Card title="Effect-TS Architecture" href="/internal/architecture/effect-ts" icon="layer-group">
