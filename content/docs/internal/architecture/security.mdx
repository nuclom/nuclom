...
Rate limits are configured in `packages/lib/src/rate-limit.ts` and applied via Next.js middleware:
...
CORS is strictly configured in `packages/auth/src/auth.ts`:
...
```
packages/lib/src/rate-limit.ts                        # Rate limiting implementation
packages/lib/src/redis.ts                             # Redis client configuration
packages/auth/src/auth.ts                             # Authentication configuration
packages/lib/src/access-control.ts                    # RBAC configuration
packages/lib/src/audit-log.ts                         # Audit logging service
apps/saas/src/middleware.ts                           # Next.js middleware
apps/saas/next.config.ts                              # Security headers
apps/saas/src/app/(api)/api/user/sessions/            # Session management API
apps/saas/src/app/(api)/api/organizations/[id]/sso/   # SSO configuration API
apps/saas/src/app/(api)/api/organizations/[id]/roles/ # RBAC roles API
apps/saas/src/app/(api)/api/organizations/[id]/audit-logs/ # Audit logs API
```
