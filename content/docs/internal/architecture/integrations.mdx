...
The integrations use Effect-TS for type-safe API operations. SDK clients are created via Effect services (for example `GoogleClient`, `SlackClient`, `GitHubClient`, `NotionClient`, `MicrosoftTeamsClient`, `ZoomClient`, and `AssemblyAIClient`) and are provided via Layers so all API calls share consistent lifecycle management and error handling.

```
src/lib/effect/services/
├── zoom-client.ts             # Zoom SDK client factory (@zoom/rivet)
...
All integration API calls use official, recommended SDKs where available:

- GitHub: `octokit` + `@octokit/graphql` + `@octokit/oauth-methods`
- Slack: `@slack/web-api` + `@slack/webhook`
- Notion: `@notionhq/client`
- Google: `googleapis`
- Microsoft Teams: `@azure/msal-node` + `@microsoft/microsoft-graph-client`
