...
description: "Setting up and managing the PostgreSQL database for Nuclom using Drizzle ORM"
icon: "database"
---
...
<Tip>
See [schema.ts](https://github.com/SferaDev/nuclom/blob/main/src/lib/db/schema.ts) for complete schema definition.
</Tip>
...
<Steps>
  ...
  <Step title="Generate Schema and Migrate">
    ```bash
...
<Steps>
  ...
  <Step title="Review migration">
    Check the generated file in `drizzle/` directory
  </Step>
...
<Tabs>
  ...
  <Tab title="Railway">
    <Steps>
      <Step title="Create database">Create new PostgreSQL database</Step>
      <Step title="Copy connection">Copy connection string from dashboard</Step>
      <Step title="Set environment">Set `DATABASE_URL` in environment variables</Step>
    </Steps>
  </Tab>
  <Tab title="Supabase">
    <Steps>
      <Step title="Create project">Create new project</Step>
      <Step title="Get connection">Go to Settings -> Database</Step>
      <Step title="Copy string">Copy connection string</Step>
      <Step title="Use pooling">Use connection pooling for production</Step>
    </Steps>
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Schema Sync Issues">
    ```bash
...
<CardGroup cols={2}>
  ...
  <Card title="Environment Config" icon="gear" href="/internal/reference/environment-config">
